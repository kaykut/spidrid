...
- [x] (2026-01-20 19:45 UTC) Milestone 1: Enhance existing adapters with comprehensive language-specific prefixes
  - [x] Research and compile enhanced prefix lists for Spanish, French, Italian, Portuguese
  - [x] Update SpanishAdapter with enhanced prefixes (9 language-specific, 53 total)
  - [x] Update FrenchAdapter with enhanced prefixes (13 language-specific, 57 total)
  - [x] Update ItalianAdapter with enhanced prefixes (7 language-specific, 51 total)
  - [x] Update PortugueseAdapter with enhanced prefixes (10 language-specific, 54 total)
  - [x] Update tests to verify enhanced prefixes exist (12 new tests, 74 total passing)
...
- **Milestone 1 - Class initialization order with spread operator**: Initial attempt to use `readonly compoundPrefixes: string[] = [...super.compoundPrefixes, ...]` failed with "undefined is not iterable" error. JavaScript class property initializers run before the parent class is fully initialized, so `super` is not accessible. Solution: Use getter accessor (`get compoundPrefixes()`) instead of property initializer.
  Evidence: Test suite failed with TypeError at SpanishAdapter.ts:30 until changed to getter pattern.
  Impact: All adapters now use getters for compoundPrefixes, which is actually more memory-efficient (creates array on-demand vs storing).
...
- Decision: Use getter accessors instead of property initializers for compoundPrefixes
  Rationale: JavaScript class property initializers cannot access `super` before parent class is fully initialized. Attempting `readonly compoundPrefixes = [...super.compoundPrefixes, ...]` throws "undefined is not iterable". Getter pattern (`get compoundPrefixes()`) resolves this by deferring array construction until access time. Bonus: More memory-efficient (creates array on-demand vs storing).
  Date: 2026-01-20 (Milestone 1 implementation)

- Decision: Export BASE_LATIN_PREFIXES as a constant from BaseLatinAdapter
  Rationale: Provides cleaner alternative to `super.compoundPrefixes` access. Improves code clarity and enables potential reuse. BaseLatinAdapter's compoundPrefixes getter returns this constant, and child adapters spread it directly.
  Date: 2026-01-20 (Milestone 1 implementation)

- Decision: Change BaseLatinAdapter.compoundPrefixes from property to getter
  Rationale: TypeScript error TS2611: "compoundPrefixes is defined as a property in BaseLatinAdapter, but is overridden as an accessor in child classes." To maintain type consistency across inheritance hierarchy, all classes must use same property descriptor type (all getters or all properties). Getter chosen for reasons stated above.
  Date: 2026-01-20 (Milestone 1 implementation)
